...
# =========================
# ✅ FORCE RETRAIN FLAG (CHANGE HERE)
# =========================
...
def regression_report(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    """
    Standard regression metrics + "accuracy-like" metrics.
    RMSE computed in a sklearn-version-safe way.
    """
    ...
    # ✅ Version-safe RMSE (no squared= argument)
...
def ensure_user_input_template(df: pd.DataFrame):
    ...
    print(f"\n⚠️  Created {USER_INPUT_PATH}. Edit it with your workload values, then run `py rf.py` again.\n")
...
def load_or_train():
    ...
    if not os.path.exists(CSV_PATH):
        raise FileNotFoundError(f"Couldn't find {CSV_PATH}. Put it in this folder or edit CSV_PATH in rf.py.")
    ...
    return preprocessor, models, target_ranges, DEFAULT_WEIGHTS


def recommend(preprocessor, models, user_context, target_ranges, weights):
...
def main():
    ...
    best, ranked = recommend(preprocessor, models, user_context, target_ranges, weights)
